...
export default function KYBSection() {
  ...
  const [accountInfo, setAccountInfo] =
    useState<StripeConnectAccountInfo | null>(null);
  ...
  const getVerificationProgress = () => {
    ...
    const possibleTotal =
      totalReqs + (requirements.past_due?.length || 0) * (pastDueWeight - 1);
    ...
    if (!accountInfo.detailsSubmitted) {
      return (
        <Alert variant='default' className='mb-4'>
          ...
          <AlertDescription>
            Pour recevoir des paiements en ligne, vous devez vérifier votre
            entreprise avec Stripe.
          </AlertDescription>
        </Alert>
      );
    }
          ...
          <div className='space-y-2 max-w-xs'>
            ...
            {allPendingRequirements.length > 0 ? (
              <ul className='space-y-2'>
                {allPendingRequirements.map(
                  ({ req, type, label, color }, index) => (
              ...
              <p className='text-sm text-muted-foreground'>
                Votre dossier est en cours d&apos;examen par Stripe. Vous
                recevrez bientôt une notification.
              </p>
            )}
