...
export const createAnimalSchemaAction = createServerAction(
  ...
  async (input, ctx) => {
    ...
    const newPet = await db.insert(pets).values({
      ...
      type: input.type === "DOG" ? "Dog" : 
            input.type === "CAT" ? "Cat" : 
            input.type === "HORSE" ? "Horse" : 
            input.type === "BIRD" ? "Bird" : "NAC",
  ...
  async (input, ctx) => {
    ...
    const updatedPet = await db.update(pets)
      .set({
        ...
        type: input.data.type === "DOG" ? "Dog" : 
              input.data.type === "CAT" ? "Cat" : 
              input.data.type === "HORSE" ? "Horse" : 
              input.data.type === "BIRD" ? "Bird" : 
              input.data.type === "NAC" ? "NAC" : undefined,
...
export const getReportsAction = createServerAction(
  ...
  async (input, ctx) => {
    ...
    const reports = input.petId 
      ? allReports.filter(r => r.appointments?.patientId === input.petId)
      : allReports;
