...
import EventModalClient from "./event-modal-client"

interface SideDrawerClientProps {
  isOpen: boolean
  onClose: () => void
...
const SideDrawerClient = ({
  ...
  const handleSaveEvent = (event: Event) => {
    ...
    if (isOverlapping) {
      alert("Ce rendez-vous chevauche un rendez-vous existant. Veuillez choisir un autre horaire.")
      return
    }
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:w-[540px] p-6">
        ...
        <ScrollArea className="h-[calc(100vh-200px)]">
          {events.length === 0 ? (
            <p className="text-center text-gray-500 dark:text-gray-400">Aucun rendez-vous pour ce jour.</p>
          ) : (
              ...
              {events.map(event => (
                <div key={event.id} className={`p-4 rounded-lg shadow-md ${eventColors[event.category]}`}>
                  ...
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => onDeleteEvent(event)}>
