...
const SlotsForm = ({ onSubmit: onFormSubmit, onCancel, initialData, isEditing, selectedSlotId }: SlotsFormProps) => {
  const [currentStep, setCurrentStep] = React.useState<keyof typeof steps>(isEditing ? "service" : "type")
  ...
  const handleDayToggle = (dayId: string) => {
    ...
    setValue(
      ...
      currentDays.includes(dayId) ? currentDays.filter(d => d !== dayId) : [...currentDays, dayId],
      { shouldValidate: true }
    )
  }
    ...
    <div className="grid grid-cols-2 gap-4 h-full">
      <button
        ...
        className={cn(
          ...
          type === "unique" ? "border-2 border-primary" : "hover:border-primary/50"
        )}
      >
        <div className="flex flex-col justify-between h-full w-full">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Créez un créneau pour une date spécifique</p>
            </div>
          </div>
        </div>
      </button>
      ...
      <button
        ...
        className={cn(
          "h-full text-left p-6 rounded-xl border transition-all flex items-stretch",
          type === "recurring" ? "border-2 border-primary" : "hover:border-primary/50"
        )}
      >
        <div className="flex flex-col justify-between h-full w-full">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Créez des créneaux qui se répètent chaque semaine</p>
            </div>
          </div>
        </div>
      </button>
    ...
    <div className="space-y-6">
          ...
          {services?.data?.map(service => (
            <button
              ...
              className={cn(
                ...
                serviceId === service.id ? "border-2 border-primary" : "hover:border-primary/50"
              )}
            >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <TimePicker value={startTime} onChange={value => setValue(`slots.0.startTime`, value)} />
        </div>
        <div>
          <Label className="text-sm font-medium mb-2 block">Heure de fin</Label>
          <TimePicker value={endTime} onChange={value => setValue(`slots.0.endTime`, value)} />
        </div>
      </div>
    </div>
  )
  ...
  const renderDateStep = () => (
    <div className="space-y-6">
      {type === "unique" ? (
        ...
        <>
          <div>
            <Label className="text-sm font-medium mb-2 block">Jours de récurrence</Label>
          ...
          {services?.data && serviceId && (
            <div className="mt-4 p-4 rounded-xl bg-muted">
              <p className="text-sm text-muted-foreground">
                {calculateNumberOfSlots()} créneaux de {services.data.find(s => s.id === serviceId)?.duration} minutes
                seront créés entre {startTime} et {endTime}
                les {selectedDays?.map(d => weekDays.find(w => w.id === d)?.label).join(", ")}
                {date &&
                  endRecurrence &&
                  ` du ${date.toLocaleDateString("fr-FR")} au ${endRecurrence.toLocaleDateString("fr-FR")}`}
              </p>
            </div>
          )}
        </>
  ...
  return (
    ...
    >
              ...
              <div
                className={cn(
                  ...
                  currentStep === step.id
                    ...
                    : index < Object.values(steps).findIndex(s => s.id === currentStep)
      ...
      <div>
        <h2 className="font-semibold text-lg">{isEditing ? "Modifier le créneau" : steps[currentStep].title}</h2>
        <p className="text-sm text-muted-foreground">
          {isEditing ? "Modifiez les paramètres de votre créneau" : steps[currentStep].description}
        </p>
      </div>

      <div className="max-h-[500px] overflow-y-auto">
        {!isEditing && currentStep === "type" && renderTypeStep()}
        {((!isEditing && currentStep === "service") || isEditing) && renderServiceStep()}
        {((!isEditing && currentStep === "date") || (isEditing && type === "recurring")) && renderDateStep()}
      </div>
      ...
      <div className="flex justify-between pt-4">
        ...
        >
          {isEditing ? "Enregistrer" : isLastStep ? "Créer les créneaux" : "Suivant"}
        </Button>
      </div>
    </form>
  )
