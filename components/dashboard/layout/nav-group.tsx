    ...
    <SidebarGroup>
      ...
      <SidebarGroupContent>
        {menus.map(item => {
          ...
          if (!item.submenus) return <SidebarMenuLink key={key} item={item} href={href} />
...
const SidebarMenuLink = ({ item, href }: { item: Menu; href: string }) => {
  ...
  return (
    <SidebarMenuItem key={item.label}>
      <SidebarMenuButton asChild isActive={checkIsActive(href, item)} tooltip={t(item.label)}>
...
const SidebarMenuCollapsible = ({ item, href }: { item: Menu; href: string }) => {
  ...
  return (
    <Collapsible key={item.label} asChild defaultOpen={checkIsActive(href, item, true)} className="group/collapsible">
      <SidebarMenuItem key={t(item.label)}>
        <CollapsibleTrigger asChild>
          <SidebarMenuButton tooltip={t(item.label)} isActive={checkIsActive(href, item)}>
            {item.icon && <item.icon />}
            <span>{t(item.label)}</span>
        ...
        <DropdownMenuContent side="right" align="start" sideOffset={4}>
          ...
          {item.submenus?.map(sub => (
            <DropdownMenuItem key={`${t(sub.label)}-${sub.href}`} asChild>
              <Link
                href={sub.href}
                className={cn("flex items-center gap-2", checkIsActive(href, sub) ? "bg-secondary" : "")}
              >
                {sub.icon && <sub.icon className="h-4 w-4 shrink-0 stroke-[2]" />}
...
function checkIsActive(href: string, item: Menu, mainNav = false) {
  return (
    ...
    (mainNav && href.split("/")[1] !== "" && href.split("/")[1] === item?.href?.split("/")[1])
  )
}
