...
export function AccountSwitchDialog({
  ...
  const bgColor = isProfessional ? "bg-primary/30 dark:bg-primary-900/30" : "bg-secondary/30 dark:bg-secondary-900/30"
  const borderColor = isProfessional ? "border-primary/50" : "border-secondary/50"
  ...
  // Textes plus amicaux
  const title = isProfessional ? `${organizationName || "Espace pro"}` : "Espace personnel"
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className={cn("border p-4 max-w-sm shadow-md rounded-xl", borderColor, bgColor)}>
        <div className="flex items-center gap-4">
          <div className={cn("rounded-full p-2 border", iconBgColor, borderColor)}>
            {isLoading ? (
          ...
          {!isLoading && (
            <div className={cn("rounded-full p-1", isProfessional ? "bg-primary" : "bg-secondary")}>
