    ...
    >
      <div className="relative h-full flex flex-col justify-start py-4">
        <div className={"flex justify-start items-center mx-4 gap-4"}>
          <MoveLeft
            className={cn(
              "h-5 w-5 text-gray-600 hover:text-gray-400",
              sidebar.isOpen == true ? "visible" : "hidden"
            )}
            onClick={() => router.push("/")}
          />
          <DropdownMenu onOpenChange={setIsOpen}>
            <DropdownMenuTrigger asChild>
              ...
              >
                ...
                <ChevronDown
                  className={cn(
                    "h-4 w-4 transition-transform ease-in-out duration-200",
