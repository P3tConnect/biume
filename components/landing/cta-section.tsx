...
export default function CallToActionSection() {
  ...
  return (
    ...
    >
      <div className="rounded-[32px] w-full bg-gray/75 bg-transparent/10 border-1 border-[#D8D8D8] px-6 md:px-20 py-6 md:py-10 mx-auto dark:bg-black/30 dark:border-gray-300/50">
        <div className="flex flex-col xl:flex-row justify-between items-center md:items-center content-center text-start gap-8 md:gap-16">
          ...
          <div className="rounded-[32px] bg-white h-auto md:h-[368px] w-full md:w-[712px] p-6 md:p-10 text-[20px] md:text-[24px]">
            <div className="flex flex-col text-center items-center justify-center content-center text-black">
              <p>Rejoignez-nous et inscrivez-vous <br/><span className="font-extrabold">sur la liste d&apos;attente</span></p>
              <form className="w-full gap-3 flex flex-col items-center justify-center mt-6 md:mt-10" onSubmit={onSubmit}>
                <Input
                  placeholder="Email"
                ...
                />
                {errors.email && (
                  <p className="font-medium text-red-400">{errors.email.message}</p>
                )}
                {isLoading ? (
                  <Loader/>
                ) : (
