...
import { webhookHasData, webhookHasMeta } from "@/src/lib/typeguards";
import { takeUniqueOrThrow } from "@/src/lib/utils";
...
export async function syncPlans() {
  ...
  const productVariants: NewPlan[] = await db.select().from(plan);
  ...
  async function _addVariant(variant: NewPlan) {
    ...
    await db
      .insert(plan)
      .values(variant)
      .onConflictDoUpdate({ target: plan.variantId, set: variant });
...
export async function storeWebhookEvent(
  ...
  const returnedValue = await db
    ...
    .onConflictDoNothing({ target: plan.id })
    .returning();
  ...
  if (!webhookHasMeta(eventBody)) {
    ...
    if (webhookEvent.eventName.startsWith("subscription_payment_")) {
      ...
      const planItem = await db
        .select()
        .from(plan)
        .where(eq(plan.variantId, parseInt(variantId, 10)));
      ...
      if (planItem.length < 1) {
        ...
        const updateData: NewSubscription = {
          ...
          planId: planItem[0].id,
        };

        // Create/update subscription in the database.
        try {
          await db.insert(dbSub).values(updateData).onConflictDoUpdate({
            target: dbSub.lemonSqueezyId,
            set: updateData,
          });
        } catch (error) {
...
export async function cancelSub(id: string) {
  ...
  try {
    await db
      ...
      .where(eq(dbSub.lemonSqueezyId, id));
  } catch (error) {
...
export async function changePlan(currentPlanId: number, newPlanId: number) {
  ...
  try {
    await db
      ...
      .where(eq(dbSub.lemonSqueezyId, subscription.lemonSqueezyId));
  } catch (error) {
    throw new Error(
