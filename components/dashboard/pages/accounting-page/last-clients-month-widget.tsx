    ...
    <Card className="w-full h-full flex flex-col rounded-3xl">
      <CardHeader className='flex flex-row w-full justify-between'>
        ...
        <div className='flex flex-row gap-3'>
          <Button variant="outline" className='border border-secondary bg-transparent text-secondary hover:bg-secondary hover:text-white dark:text-white dark:border dark:border-white dark:hover:border-secondary' size="sm">
            ...
            {clients.slice(0, maxVisibleClients).map((client, index) => (
              <li key={index} className="flex items-center gap-4 justify-between">
                <div className='flex flex-row gap-2 items-center'>
                  <Image src={client.image} alt={client.name} className="w-10 h-10 rounded-full" width={20} height={20} />
